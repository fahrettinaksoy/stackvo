...
Her Stackvo projesi için `stackvo.json` dosyası gereklidir. Bu sayfa, proje bazlı konfigürasyon dosyasının tüm alanlarını, PHP ve diğer runtime versiyonlarını, server seçeneklerini, document root ayarlarını ve çoklu dil desteğini detaylı olarak açıklamaktadır. Proje konfigürasyonu, her projenin bağımsız olarak özelleştirilmesini sağlar.
