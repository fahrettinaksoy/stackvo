...
source "$MODULE_DIR/project/dockerfile/frankenphp.sh"
source "$MODULE_DIR/project/dockerfile/swoole.sh"
...
source "$MODULE_DIR/project/compose/frankenphp.sh"
source "$MODULE_DIR/project/compose/swoole.sh"

# Load PHP extensions library (needed for extension metadata)
PHP_EXT_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$PHP_EXT_LIB_DIR/../php-extensions.sh"
...
generate_single_dockerfile() {
    ...
    local pecl_install=""
    local count_builtin=0
    local count_pecl=0
    local count_standard=0
    local count_skipped=0

    for ext in $extensions; do
        # Skip Composer packages
        if is_composer_package "$ext"; then
            log_debug "[$project_name] Skipping Composer package: $ext"
            ((count_skipped++))
            continue
        fi

        # Skip built-in extensions (PHP 8.0+)
        if is_builtin_extension "$ext" "$php_version"; then
            log_debug "[$project_name] Extension '$ext' is built-in (PHP $php_version)"
            ((count_builtin++))
            continue
        fi
        ...
        if is_pecl_extension "$ext"; then
            log_debug "[$project_name] Adding PECL extension: $ext"
            ((count_pecl++))
        else
            log_debug "[$project_name] Adding standard extension: $ext"
            ((count_standard++))
        fi
    ...
    case "$web_server" in
        apache)
            generate_apache_dockerfile "$dockerfile" "$php_version" "$apt_packages" "$configure_commands" "$docker_ext_install" "$pecl_install" "$project_name" "$project_dir" "$document_root"
            ;;
        nginx)
        ...
        caddy)
            ...
            ;;
        frankenphp)
            generate_frankenphp_dockerfile "$dockerfile" "$php_version" "$apt_packages" "$configure_commands" "$docker_ext_install" "$pecl_install" "$project_name" "$project_dir" "$document_root"
            ;;
        swoole)
            generate_swoole_dockerfile "$dockerfile" "$php_version" "$apt_packages" "$configure_commands" "$docker_ext_install" "$pecl_install" "$project_name" "$project_dir" "$document_root"
            ;;
        *)
    ...
    log_debug "Dockerfile created for $project_name: $dockerfile"
...
generate_single_container() {
    ...
    case "$web_server" in
        ...
        caddy)
            ...
            ;;
        frankenphp)
            generate_frankenphp_single_container "$project_name" "$project_path" "$project_domain" "$document_root" "$host_project_path" "$host_logs_path" "$host_generated_configs_dir" "$host_generated_projects_dir"
            ;;
        swoole)
            generate_swoole_single_container "$project_name" "$project_path" "$project_domain" "$document_root" "$host_project_path" "$host_logs_path" "$host_generated_configs_dir" "$host_generated_projects_dir"
            ;;
        *)
            log_warn "Unknown web server: $web_server, using nginx"
            generate_nginx_single_container "$project_name" "$project_path" "$project_domain" "$host_project_path" "$host_logs_path" "$host_generated_configs_dir" "$host_generated_projects_dir"
