...
##
# Determine Apache config mount path
# Only mounts custom config if user provides one.
# Otherwise, the Dockerfile handles Apache configuration via sed.
#
# Parameters:
...
get_apache_config_mount() {
    ...
    # Does .stackvo/apache.conf exist? (custom override)
    ...
    # Does apache.conf exist in project root? (custom override)
    if [ -f "$project_path/apache.conf" ]; then
        echo "      - ${host_project_path}/apache.conf:/etc/apache2/sites-available/000-default.conf:ro"
        return 0
    fi

    # No custom config - Dockerfile handles it (sed replaces DocumentRoot)
    echo ""
}
