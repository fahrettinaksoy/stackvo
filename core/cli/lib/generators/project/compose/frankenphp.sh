#!/bin/bash
###################################################################
# STACKVO FRANKENPHP COMPOSE GENERATOR MODULE
# FrankenPHP compose service generation
###################################################################

##
# Generate FrankenPHP single container compose service
#
# Parameters:
#   $1 - Project name
...
##
generate_frankenphp_single_container() {
    ...
    # Determine Caddyfile mount path (same pattern as Caddy)
...
EOF
    ...
    # Traefik labels (port 80 - FrankenPHP listens on 80 by default)
