...
parse_project_config() {
    ...
    local web_server=$(sed -n 's/.*"server"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$project_json")
    if [ -z "$web_server" ]; then
        # Fallback to legacy "webserver" field for backward compatibility
        web_server=$(sed -n 's/.*"webserver"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "$project_json")
    fi
    ...
    # Parse extension array (pure bash - multi-line support)
    # Extract all extension values from the extensions array (handles multi-line JSON)
    local extensions=$(grep -A 50 '"extensions"' "$project_json" | grep -o '"[a-z_0-9]*"' | sed 's/"//g' | grep -v extensions | tr '\n' ' ')
    ...
    if [ -z "$web_server" ]; then
        log_warn "Server not found ($project_json), using default: ${DEFAULT_SERVER:-$CONST_DEFAULT_SERVER}"
        web_server="${DEFAULT_SERVER:-$CONST_DEFAULT_SERVER}"
    fi
...
validate_project_config() {
    ...
    case "$web_server" in
        nginx|apache|caddy|frankenphp|swoole)
            return 0
            ;;
        *)
